...
# Load graph edges
inner_edge = np.array(np.load("./inner_edge.npy"))
inner10_edge = np.array(np.load("./inner10_edge.npy"))
inner20_edge = np.array(np.load("./inner20_edge.npy"))
outer_edge = np.array(np.load("./outer_edge.npy"))

# Load category mapping to get actual number of stocks and categories
with open("category_mapping.pickle", "rb") as f:
    category_info = pickle.load(f)
    num_stocks = category_info['num_stocks']
    num_categories = category_info['num_categories']
...
def train(args):
    ...
    if model_name == "CG":
        ...
        raise ValueError(f"Unknown model: {model_name}. Choose from [CG, CAT, CPool]")
    ...
    for epoch in range(epochs):
        for week in range(num_weeks):
            ...
            if (week+1) % 1 == 0:
                ...
                ra_loss = torch.tensor([]).float().to(device)
                if (week+1) % 50 == 0:  
                    print("Epoch %d Week %d - REG Loss: %.4f CLS Loss: %.4f RANK Loss: %.4f Total Loss: %.4f" % 
                          (epoch+1, week+1, reg_loss.item(), cls_loss.item(), rank_loss.item(), loss.item()))
        ...
        mae = round(mean_absolute_error(test_y_flat, y_pred), 4)
        ...
        for k in ks_list:
            ...
            for i in range(test_shape):
                M = MRR(np.array(test_y_flat[loop_number*i:loop_number*(i+1)]), 
                       np.array(y_pred[loop_number*i:loop_number*(i+1)]), k=k)
                MRRs.append(M)
                P = Precision(np.array(test_y_flat[loop_number*i:loop_number*(i+1)]), 
                             np.array(y_pred[loop_number*i:loop_number*(i+1)]), k=k)
                Prs.append(P)
            over_all = [mae, round(acc_score, 4), round(np.mean(MRRs), 4), round(np.mean(Prs), 4)]
            results.append(over_all)
        
        print(f"Results for k={ks_list}: {results}")
        ...
        if np.mean(MRRs) > global_best_MRR:
            ...
            print(f"New best MRR: {global_best_MRR:.4f}")
...
if __name__ == "__main__":
    ...
    print("="*50)
    print("\n-------Final result-------")
    if best_metric_MRR is not None:
        print("[BEST MRR] MAE: %.4f ACC: %.4f MRR: %.4f Precision: %.4f" % tuple(best_metric_MRR))
        for idx, k in enumerate(ks_list):
            print("[BEST RESULT MRR with k=%s] MAE: %.4f ACC: %.4f MRR: %.4f Precision: %.4f" % 
                  tuple(tuple([k])+tuple(best_results_MRR[idx])))
    else:
        print("No best result found during training.")
